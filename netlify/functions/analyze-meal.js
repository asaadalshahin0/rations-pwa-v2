exports.handler = async (event) => {
  try {
    ...
    const prompt = [
      ...
      cleanFoodMemory ? `Persistent user food memory learned from meals the user previously logged. Treat these foods, portions, and macros as authoritative when the current meal mentions or visually matches them; ignore unrelated entries. ${cleanFoodMemory}` : 'No persistent user food memory saved.',
